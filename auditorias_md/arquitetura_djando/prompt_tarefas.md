...
ATENÇÃO - ESCRITA DE ARQUIVOS EXTENSOS:
Para gravar documentos longos como o plano de tarefas, **NUNCA** utilize o terminal/PowerShell para injetar texto longo em arquivos via script (isso excede o limite de buffer do Windows). **NUNCA** use parâmetros de `ArtifactMetadata` se estiver utilizando a ferramenta de escrita para criar arquivos diretamente nas pastas do workspace (como `mds/`).
**SEMPRE** utilize a ferramenta direta `write_to_file` em modo *overwrite* fornecendo o caminho local sem ativar opções de artefatos da interface, permitindo assim escrever documentos gigantescos de forma segura.
</regras_fixas>

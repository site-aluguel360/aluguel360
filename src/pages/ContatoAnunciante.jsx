...
function Avatar({ tamanho = 36, preenchido = false }) {
  return (
    <div
      ...
      className={`flex flex-shrink-0 items-center justify-center rounded-full border-2 border-white shadow-sm ${preenchido ? "bg-teal-700 text-white" : "border-teal-300 bg-teal-50 text-teal-700"
        }`}
    >
...
function MensagemChat({ mensagem, ehUsuario }) {
  return (
    <div className={`flex flex-col ${ehUsuario ? "items-end" : "items-start"}`}>
      ...
      <div
        className={`max-w-[75%] rounded-xl px-3.5 pb-2.5 pt-4 text-sm leading-relaxed ${ehUsuario ? "bg-teal-50 text-gray-800" : "bg-gray-100 text-gray-800"
          }`}
      >
        <p>{mensagem.texto}</p>
      </div>
...
function ChatContato({ imovel, locador, usuario }) {
    ...
    setTimeout(() => {
      ...
      setConversa((prev) => [
        ...prev,
        {
          ...
          autor: "locador",
          texto: respostas_locador[respostaIndice % respostas_locador.length],
          hora: horaAtual(),
          grupo: "Hoje",
        },
      ]);
  ...
  return (
    <div className="flex h-[100dvh] w-full flex-col bg-white sm:mx-auto sm:h-[85vh] sm:max-w-2xl sm:overflow-hidden sm:rounded-2xl sm:border sm:border-gray-100 sm:shadow-lg">
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
        {conversa.map((m, index) => {
          const mostrarDivisor = index === 0 || m.grupo !== conversa[index - 1].grupo;
          return (
            <div key={m.id}>
...
export { PaginaImovel as ContatoAnunciante };
